...
export default class ServiceComponent extends React.Component {
    ...
    start() {
        ...
        if(typeof indi !== 'undefined') {
            this.setState({imageid: this.state.array[indi][1]});
            this.setState({title: this.state.array[indi][2]});
            this.setState({shorttext: this.state.array[indi][3]});
            this.setState({longtext: this.state.array[indi][4]});
            this.setdot(indi);
        }else {
            ...
            <div className="servicediv">
                ...
                <div className="servicetop">
                    ...
                    <div className="servicetop-right">
                        <h1 id="serviceHeadline">{this.state.array[this.state.index][2]}</h1>
                        <p>{this.state.array[this.state.index][3]}</p>
                        <div id="dotdiv" className="dotdiv">
                            <span className={this.state.dotclasses[0]} onClick={() => this.setdot(0)}/>
                            <span className={this.state.dotclasses[1]} onClick={() => this.setdot(1)}/>
                            <span className={this.state.dotclasses[2]} onClick={() => this.setdot(2)}/>
                            <span className={this.state.dotclasses[3]} onClick={() => this.setdot(3)}/>
                            <span className={this.state.dotclasses[4]} onClick={() => this.setdot(4)}/>
                        </div>
                    </div>
                </div>
